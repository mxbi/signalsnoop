...
# ╔═╡ adea5f86-7c61-11ed-14dd-15a5a423e863
using DSP, WAV, FFTW, Plots, Colors, ImageIO, FileIO, ImageShow, StatsBase;

# ╔═╡ d76464c7-2be4-438b-9064-317eb9968b11
function load_iq(fn)
...
# ╔═╡ 5ff868f0-4fee-42a3-9c66-6aace834af53
begin
	const Fs = 64_000_000
	const Fp = 25_175_000
	const Xt = 800
...
# ╔═╡ 6a65e5b5-9865-42f2-bc60-1292bf27f286
"""
...
# ╔═╡ 8ea4c381-50ac-4cbe-9787-b88d5f97be74
...
# ╔═╡ ffaf9523-930c-418d-b880-a3a128eed159
...
# ╔═╡ 21de217e-3414-486e-bb1c-8e3d93c68c59
...
# ╔═╡ 15d33774-d07e-4216-85e3-9ed4b889c088
"Given an input signal, recover a pixel clock in Hz by correlating blanking intervals." 
...
# ╔═╡ 36d1c6fb-53ad-4570-9e86-d2381ac24d89
...
# ╔═╡ b2d6ff7b-03c6-495f-b530-fb7806bc2faf
norm1(data) = minmaxnorm(data, 0, 1)

# ╔═╡ f8e7cd40-12d9-42c7-bc68-3cfa3ddad77d
...
# ╔═╡ 63283feb-1c70-4abe-9040-91a90cbbcf06
...
# ╔═╡ 03ad81ef-ecf8-4f97-b656-d1558085ba0b
...
# ╔═╡ dd5981f3-84d1-47aa-9498-152f8e803432
...
# ╔═╡ 868b5de9-fbdf-44d4-8d6c-1b5f8b4b2184
...
# ╔═╡ bbec6a5c-9db5-41cd-b264-8169878b9972
md"""
## Working on the first signal again
"""

# ╔═╡ 0d995f5e-f481-402d-99ea-49d97de052d2
data = load_iq("./data/scene3-640x480-60-425M-64M-40M.dat")

# ╔═╡ ab37b47c-2e69-481b-a97c-219eb826fc16
let
...
# ╔═╡ fb2005d0-733c-40aa-9d1f-774869751e6c
let
...
# ╔═╡ 89f96a41-3f74-451b-8d9a-39facc57fd94
let
	riq = resample(abs.(data), Fs, 25_200_175*SAMPLE_FACTOR, interp=:linear)
...
# ╔═╡ 30f59c78-16e4-495a-913b-3203974cbbf3
md"""
## Detecting pixel clock and offset automatically
"""

# ╔═╡ 6c2f58ae-f610-4439-b44f-aad2380deb27
Fp2 = trunc(Int, recoverClock2(data, [1, 100, Yt, Yt*4, Yt*16])) #trunc(Int, recoverClock(data, nFrames=10))

# ╔═╡ 621f6594-aec6-4f86-9b7f-8ca91999e412
offset = let 
	sol, sof = detectBlankingIntervals(data, Fp2)
	sof*Xt + sol
end

# ╔═╡ 8d2f1ea8-bd9f-4cd5-a2f2-8507bcb61612
let
...
# ╔═╡ 94c73a38-cc5b-4d0b-b295-2c7356aacabe
let
	# Average of 15 frames
	riq = resample(abs.(data), Fs, Fp2, interp=:linear)
	avgframe = coherentavg(rowify(riq[offset+1:end], Xt*SAMPLE_FACTOR)', 50)
	Gray.(norm1(avgframe)*1.5)
end

# ╔═╡ 785b8c06-5019-473b-884a-6d0a964bfa53
md"""
## IQ analysis
"""

# ╔═╡ a9f6ac55-fc21-4e4f-8b07-7c402840f4b4
peak_freq = let
...
# ╔═╡ 54e6bdf7-d72a-4a38-9bf9-e5ffc527733e
let
	data_mat = rowify(resample(data_unrotated, Fs, Fp2, interp=:linear)[offset:end], Xt)'
	img = HSV.(minmaxnorm(angle.(data_mat), 0, 360).+50, 1, 1)
	img'
...
# ╔═╡ 50086be2-5ae0-4134-b2c7-db069bfbbd92
let
...
# ╔═╡ 7393c592-c5f0-4f21-95c8-f373448e7c30
function resample2(x, fs, fr; phaseOffset=0, interp=:linear)
	step = fs / fr
	indices = collect(1:step:length(x)) .+ phaseOffset
	if interp == :nearest
		indices = Int.(round.(indices))
		x[indices]
	elseif interp == :linear
...
function iqImage(num)
	...
	Fp2 = trunc(Int, recoverClock2(data, [1, Yt, Yt*4, Yt*16]))
	...
	data_mat = rowify(resample(data_unrotated[1:end], Fs, Fp2, interp=:linear)[offset:end], Xt)'
	data_mat = coherentavg(data_mat, 5)
	img = HSV.(minmaxnorm(angle.(data_mat), 0, 360), 1, norm1(abs.(data_mat))*2)
