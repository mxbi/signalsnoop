...
# ╔═╡ 5eb48352-f500-4aa5-ae70-1da71559f83c
md"""
## Auto-correlations
"""

# ╔═╡ d48edba7-9a58-49d2-9749-22aa5c6da00f
begin
	# Recover vertical blanking interval
	estimated_clock = LINE_SIZE*N_LINES * (SAMPLING_FREQ / PIXEL_CLOCK)
	min_clock = trunc(Int, estimated_clock * 0.995)
	max_clock = trunc(Int, estimated_clock * 1.005)
	clock_range = collect(min_clock:max_clock)
	println(min_clock, " ", max_clock)
	#acor = autocor(abs.(data), clock_range)
end

# ╔═╡ 162faa66-4e23-4f58-abd4-2afc25c9e2d5
begin
	recovered_clock = 1 ./ (collect(min_clock:max_clock) / (LINE_SIZE*N_LINES) / SAMPLING_FREQ) / 1_000_000
	p=plot(recovered_clock, acor, xlabel="Recovered pixel clock (MHz)", ylabel="Autocorrelation")
	savefig("autocor1.pdf")
	p
end

# ╔═╡ 0d72f6eb-1fcd-40ef-834f-2bab0641a876
argmax(acor)

# ╔═╡ d1c108fc-d0ab-419e-958b-3b89182b8757
recovered_clock[argmax(acor)]

# ╔═╡ 8b84b53d-e9d1-417d-94da-5df034a8defe
1_000_000*(recovered_clock[2] - recovered_clock[1])
...
recoverClock(abs.(data))

# ╔═╡ af542806-8a57-41ee-8e00-acb2cfe4903f
x = LINE_SIZE*(SAMPLING_FREQ / PIXEL_CLOCK)

# ╔═╡ 511f88b7-0fff-4a6b-af46-a686b1afaa95
x * 500
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "9ebbd6ecd299d497d8e81e3478988af7e517327d"
